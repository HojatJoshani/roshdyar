...
import { Youtube, Sparkles, ShieldCheck, Zap, Clock } from "lucide-react";

export const dynamic = "force-dynamic";

async function getYoutubeServices() {
...
export default async function YoutubeServicesPage() {
  const services = await getYoutubeServices();
      ...
      <section className="relative overflow-hidden border-b border-border/60 bg-mesh-brand">
        ...
        <div className="container relative mx-auto px-4 py-8 sm:py-12">
          <ServiceBreadcrumb
            items={[
              { label: "خانه", href: "/" },
              { label: "خدمات", href: "/services/youtube" },
              { label: "یوتیوب" },
            ]}
            className="mb-5"
          />
          ...
          <p className="mt-3 max-w-2xl text-pretty text-sm leading-7 text-muted-foreground sm:text-base">
            افزایش بازدید واقعی ویدیوهای یوتیوب با حفظ حساب، ریتنشن مناسب و
            افت بسیار پایین. مناسب برای تقویت رتبه ویدیو در جستجو و توصیه‌ها.
          </p>
          ...
          <div className="mt-6 flex flex-wrap items-center gap-x-5 gap-y-2 text-[11px] text-muted-foreground">
            ...
            <span className="inline-flex items-center gap-1.5">
              ...
              تحویل تدریجی و طبیعی
            </span>
            <span className="inline-flex items-center gap-1.5">
              <Clock className="h-3.5 w-3.5 text-primary" />
              ریتنشن بالا در سطح حرفه‌ای
            </span>
          </div>
        </div>
      </section>
        ...
        {services.length === 0 ? (
          <div className="flex flex-col items-center justify-center rounded-3xl border border-dashed border-border bg-card/30 py-20 text-center">
            ...
            <p className="mt-1.5 max-w-sm text-sm leading-6 text-muted-foreground">
              در حال حاضر سرویس فعال یوتیوب در دسترس نیست. بعداً دوباره تلاش
              کنید.
            </p>
          </div>
        ) : (
